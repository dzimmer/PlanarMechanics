...
model Arrow
  ...
  input SI.Diameter diameter=1/40 "Diameter of arrow line"
    annotation(Dialog(enable=true));
  input SI.Diameter headDiameter=diameter*PlanarMechanics.Types.Defaults.ArrowHeadWidthFraction "Diameter of arrow head"
    annotation(Dialog(enable=true));
  input SI.Length headLength=diameter*PlanarMechanics.Types.Defaults.ArrowHeadLengthFraction "Length of arrow head"
    annotation(Dialog(enable=true));
...
protected
  ...
  MB.Visualizers.Advanced.Shape arrowHead(
    ...
    width=headDiameterMin,
    height=headDiameterMin,
    lengthDirection=to_unit1(r_head),
    widthDirection={0,1,0},
    shapeType="cone",
    color=color,
